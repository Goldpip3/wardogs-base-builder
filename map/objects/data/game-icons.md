...
- **Hits:** `/loadouts/` (an icon per slot and on every shelf card), `/armory/` (the card
  grid) and `/ballistics/` (the weapon art, the shelf, the ranking rows). Check 3e in
  `tools/check-build.js` fails on a slug with no file, a page reference with no file, a
  file outside the catalog, or a catalog entry claiming a file that is absent.
