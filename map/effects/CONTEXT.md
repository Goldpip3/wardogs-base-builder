...
| the artillery map, its terrain or its control zone | [artillery-map](../objects/site/artillery-map.md), [artillery-maps](../objects/data/artillery-maps.md) | Its client script lives in a template literal, so a backslash written there never reaches the page. The tile level is picked in device pixels, and CSS pixels means half resolution on most laptops. |
