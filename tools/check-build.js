...
// so it is exactly the kind of thing that breaks quietly.
{
  ...
  /* The site's nav is on the hosted planner and must not be in the download. Nothing in it
     fetches, so this is not the offline promise: it is that every href in it points at the
...
/* -- 3c-ii. one nav, in one order, on every page including the planner --
   The site's nav is generated by tools/site/shell.js and the planner's by build.ps1, because
   the planner is built before the site generator runs and never touches it. Two lists is how
   a page goes missing from one of them, so they are held to each other here rather than
   trusted to be copied correctly. */
{
  ...
  /* The planner names its own page rather than linking to it, so that one is expected to be
     missing from the hrefs and is put back before comparing. */
  const withSelf = plannerLinks.concat(["/planner/"]).sort();
  check(siteLinks.length >= 6, "the site nav was found to compare against",
    siteLinks.join(" "));
