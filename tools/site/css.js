...
/* One group, centred in what the brand leaves. The spacer that used to split the two
   tools off the references opened a hole most of a column wide at full width. */
nav.site{display:flex;align-items:center;justify-content:center;gap:14px;row-gap:10px;
...
/* --- questions and answers ---
   One column rather than the two the features grid uses. A question and its answer are read
   in order, and side by side they read as cards to be scanned instead. The question is set
   at reading size rather than as display type: it is a sentence, and squared uppercase at
   18px turns a sentence into a sign. */
.qa{display:grid;grid-template-columns:1fr;margin-top:20px}
.qa-item{border-top:1px solid var(--line2);padding:18px 0 20px;max-width:72ch}
.qa-item h3{font-family:var(--ui);font-weight:600;font-size:16px;margin-bottom:7px;
  text-transform:none;letter-spacing:0;color:var(--text)}
.qa-item p{font-size:14px;color:var(--dim);line-height:1.6}
.qa-item a{display:inline-block;margin-top:9px;font-weight:600;font-size:11px;
  letter-spacing:.14em;text-transform:uppercase;color:var(--dim2)}
.qa-item a:hover{color:var(--yellow)}
...
/* Buttons in a row, each carrying its own edge. They used to be cells in a grey band that
   spanned the column, so eight chips over a 1180px page drew four hundred pixels of empty
   grey beside them and the row read as a broken table rather than as a set of controls. */
.chips{display:flex;flex-wrap:wrap;gap:6px;background:none;border:0}
...
.chip{background:transparent;color:var(--dim2);border:1px solid var(--line2);cursor:pointer;
  font-family:var(--ui);font-weight:600;font-size:11px;letter-spacing:.1em;
  text-transform:uppercase;padding:8px 13px}
.chip:hover{border-color:var(--text);color:var(--text)}
.chip[aria-pressed="true"]{background:var(--yellow);border-color:var(--yellow);color:var(--bg)}
...
/* var(--accent) was written here and this stylesheet has never declared one, so the name in
   the header had no hover colour at all. The yellow is --yellow. */
.acct .who:hover{color:var(--yellow)}
...
/* The damage figure keeps the top on its own, and the three that qualify it stack under
   it: shots, time, and the rate of fire that turns one into the other. Two of them sat side
   by side at 48% while there were two; a third at 32% of a 300px column left every label
   wrapping mid-phrase, "shots to / kill" against "rounds per / minute". Full width down the
   column reads as a list of four facts about one gun, which is what it is. */
.calc-out .hero:nth-child(n+2){display:block;border-bottom:0;padding:0;margin:0 0 12px}
.calc-out .hero:last-child{margin-bottom:0}
...
.calc-out .hero:nth-child(n+2) b{font-size:24px}
...
/* The hatching sentence is the one line on this page whose length depends on what you press:
   three lines with nothing worn and five with two level 4 pieces on, measured at 128px in a
   300px column. Reserving that height means pressing a tier rewrites the words and moves
   nothing. It stacks on a narrow screen, where the column is its own height anyway and the
   reserved space would just be a gap. */
#cover{min-height:128px}
@media(max-width:899px){#cover{min-height:0}}
...
/* Two labelled rows rather than one strip of nine chips in two colour scales. The key is
   a fixed column so both rows line up under each other and the eye reads down them. */
.legend{margin-top:18px;border:1px solid var(--line);background:var(--panel);padding:12px 14px;
  display:grid;gap:8px}
.lgrow{display:grid;grid-template-columns:150px 1fr;gap:6px 14px;align-items:center}
@media(max-width:640px){.lgrow{grid-template-columns:1fr}}
.lgkey{font-size:10px;font-weight:600;letter-spacing:.16em;text-transform:uppercase;
  color:var(--dim)}
.chip .dot{display:inline-block;width:9px;height:9px;margin-right:6px;vertical-align:-1px}
/* No band, because no rate of fire has been measured: a dash in the ink of a missing thing
   rather than in one of the four colours, which would be a claim about how fast it is. */
.band[data-none="1"]{--bd:var(--line2);color:var(--dim)}
.legend .lgs{margin-top:0}
...
.rrow{display:grid;align-items:center;gap:6px 10px;padding:9px 10px;background:var(--panel);
  ...
  grid-template-areas:"name name ttk ttk" "track track track track" "load dmg stk rpm"}
/* The header is the sort. Same grid as a row so each label sits over the numbers it names,
   and pressing one orders by it: four chips above the list saying "By damage" over a column
   of unlabelled numbers was two problems, and this is one answer to both. */
.rrow.rhead{background:transparent;padding:4px 10px 6px;cursor:default;
  border-bottom:1px solid var(--line)}
.rh{background:none;border:0;padding:0;text-align:left;cursor:pointer;font-family:var(--ui);
  font-size:10px;font-weight:600;letter-spacing:.14em;text-transform:uppercase;color:var(--dim)}
.rh:hover{color:var(--text)}
.rh[aria-pressed="true"]{color:var(--yellow)}
.rh[aria-pressed="true"]{border-bottom:2px solid var(--yellow);padding-bottom:2px}
.rh-name{grid-area:name}.rh-ttk{grid-area:track}.rh-load{grid-area:load}
.rh-dmg{grid-area:dmg}.rh-stk{grid-area:stk}.rh-rpm{grid-area:rpm}
.rh-dmg,.rh-stk,.rh-rpm{text-align:right}
/* The time sits at the end of its own bar: the bar is the answer and the number is what the
   bar is worth, where a column of its own was a second place to read the same thing. */
.rtrack{position:relative;min-height:18px}
.rbarv{position:absolute;right:6px;top:50%;transform:translateY(-50%);font-family:var(--num);
  font-weight:400;font-size:11px;color:var(--text);
...
.rrow .rrpm{grid-area:rpm;color:var(--dim2)}
.rrow .rttk{grid-area:ttk}
.rh-ttk{grid-area:ttk;text-align:right}
/* Picked for comparison. The outline rather than a fill, so the bar keeps its own colour. */
.rrow[data-picked="1"]{outline:1px solid var(--yellow);outline-offset:-1px;background:var(--panel2)}
/* The caption over the bar says which column it is drawing, and is not a control. */
.rh-bar{grid-area:track;color:var(--dim);cursor:default}

/* two or three rows lifted out of the list and set side by side */
.rcmp{border:1px solid var(--yellow);background:var(--panel);margin:14px 0 0;padding:12px 14px}
.rcmp-h{display:flex;align-items:center;justify-content:space-between;gap:12px;
  font-size:10px;font-weight:600;letter-spacing:.16em;text-transform:uppercase;color:var(--dim);
  margin:0 0 10px}
.rcmp-cols{display:grid;gap:12px;grid-template-columns:1fr}
@media(min-width:640px){.rcmp-cols{grid-template-columns:repeat(auto-fit,minmax(190px,1fr))}}
.rcmp-c{border:1px solid var(--line);padding:10px 12px;background:var(--bg)}
.rcmp-n{font-family:var(--display);font-weight:700;font-size:15px;text-transform:uppercase;
  letter-spacing:.02em;margin:0 0 8px;color:var(--text)}
.rcmp-c dl{display:grid;grid-template-columns:auto 1fr;gap:4px 10px;margin:0 0 10px;font-size:12px}
.rcmp-c dt{color:var(--dim);font-size:10px;letter-spacing:.12em;text-transform:uppercase;
  align-self:center}
.rcmp-c dd{margin:0;text-align:right;font-family:var(--num);color:var(--text)}
@media(min-width:760px){
  .rrow{grid-template-columns:200px minmax(90px,1fr) 84px 58px 46px 54px 74px;
    grid-template-areas:"name track load dmg stk rpm ttk";padding:7px 10px}
  .rrow.rhead{padding:4px 10px 6px}
}
...
.rload{font-size:11px;font-weight:600;color:var(--dim2)}
...
@media(min-width:900px){.vend-body{grid-template-columns:26px 1fr 300px}}
/* The backpack shelf belongs to a 300px column and does not fit in one, so it takes a row
   under everything and spans the lot. Hidden with the picker inside it: an empty grid row
   still draws its own hairline, which read as a line ruled under the vendor for no reason. */
.vend-shelf{grid-column:1/-1;background:var(--panel);padding:0 20px 20px}
.vend-shelf[hidden]{display:none}
...
/* Two lines' worth of room, always. A slot naming an "AR Multi-Caliber Suppressor" wrapped
   to two lines, grew its own card, and pushed the row and everything under it down as you
   picked. The room is there whether the name uses it or not. */
.vslot-btn{display:block;width:100%;min-height:44px;text-align:left;background:var(--panel2);color:var(--text);
...
/* The level a thing opens at, opposite its price: what it costs and what it takes to be
   allowed to buy it are two different questions and the shelf answers both. */
/* What a bag holds, under its name on the shelf. A measured figure is marked, because the
   difference between one somebody counted in game and one pulled off a database is the
   whole reason the measured file exists. */
.vcard-sub{display:block;font-family:var(--num);font-size:10px;color:var(--dim);
  padding:0 8px 8px;text-align:center}
.vcard-sub[data-measured="1"]{color:var(--good)}
.vcard-lvl{position:absolute;top:5px;left:5px;font-family:var(--num);font-size:9px;
  color:var(--dim2);background:var(--panel2);border:1px solid var(--line2);padding:1px 4px;
...
/* --- the bag, drawn as a bag ---
   The game shows what you are carrying as a grid of cells on the right of the screen, each
   one holding the thing's own art, and a magazine reading how many rounds are in it. A list
   of names down the side said the same words and none of the same thing: you read a kit by
   looking at it. Three across at this column width, four on a phone where the column is the
   whole page and the cells shrink. */
/* The bag is drawn on the grid the game gives it, so the columns come from the bag rather
   than from the page: --cols is set when a bag is chosen. Dense flow lets a 1x2 magazine and
   a 2x2 drum pack into the gaps the way they do in game instead of each taking a row. */
.vcells{display:grid;grid-template-columns:repeat(var(--cols,3),1fr);grid-auto-rows:1fr;
  grid-auto-flow:row dense;gap:4px;background:var(--line);
  border:1px solid var(--line);padding:4px;margin-top:14px;min-height:104px;
  align-content:start}
.vcell{position:relative;background:var(--bg);border:1px solid var(--line2);
  display:flex;flex-direction:column;align-items:center;
  justify-content:center;padding:4px;overflow:hidden;min-height:52px}
.vcell img{max-width:100%;max-height:100%;object-fit:contain}
/* The count sits where the game puts it, bottom right, and reads as a number rather than as
   a label: it is 30/30 on a magazine and a multiplier on anything that stacks. */
.vcell b{position:absolute;right:3px;bottom:2px;font-family:var(--num);font-weight:400;
  font-size:9px;color:var(--yellow);background:var(--bg);padding:0 2px;line-height:1.4}
/* The name is there for a reader and a screen reader, not for the eye: at 60px square it
   would be three words of unreadable type over the art it is describing. */
.vcell span{position:absolute;width:1px;height:1px;overflow:hidden;clip-path:inset(50%)}
.vcell-empty{background:var(--panel2);border-style:dashed;border-color:var(--line)}
/* More in the bag than the bag holds. Only reachable once a capacity has been measured:
   with none, the grid grows and claims nothing. */
.vcells[data-over="1"]{border-color:var(--red)}
.vcells[data-over="1"] + .vpack-foot{color:var(--red)}
.vcells[data-locked="1"]{grid-template-columns:1fr;background:var(--panel2);
  border-style:dashed;border-color:var(--line2);min-height:96px;align-content:center}
.vcells-none{font-size:12px;color:var(--dim);text-align:center;padding:14px 12px;margin:0}
.vpack-foot{display:flex;align-items:baseline;justify-content:space-between;gap:10px;
  margin-top:10px;font-size:10px;font-weight:600;letter-spacing:.12em;
  text-transform:uppercase;color:var(--dim)}
.vpack-foot #packnote{color:var(--dim2);letter-spacing:.06em;text-transform:none;
  overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
/* A shelf you cannot buy from says so by looking shut, and its buttons are disabled rather
   than merely faded: a dimmed control you can still press is worse than no signal at all. */
[data-panel="items"][data-locked="1"] .vgrid{opacity:.4;pointer-events:none}
[data-panel="items"][data-locked="1"] .vend-hint{color:var(--yellow)}
.veq-qty[data-locked="1"]{opacity:.45}
/* The weight, which nothing has been measured for yet: it says so in the readout rather
   than printing a total that is only true of the empty set. */
.vend-cash b[data-soft="1"]{font-size:14px;color:var(--dim);font-family:var(--ui);
...
@media(max-width:1180px){
  ...
  /* flex:none matters: flex-basis:0 from the desktop rule beats width:100% inside a flex
     ...
     under it. */
  nav.site{flex:none;margin-left:0;gap:14px;width:100%;flex-wrap:wrap;justify-content:flex-start}
}
...
/* Three columns below about this width leave the map some 600px, and that is not a map you
   can read a solution off. The rail moves under it rather than being hidden: a unit that is
   requested and then display:none'd is an impression nobody can see, which is both worthless
