...
(function (root) {
  ...
  function unpackDesign(bytes, known) {
    ...
    var body = bytes.subarray(2 + headLen);
    var next = varintReader(body);
    var n = next();
    /* The piece count is read straight out of the code and then five loops run on it, so a
       crafted code could claim millions of pieces and lock the tab up before anything asked
       whether the bytes to back that up were even there. Every piece costs at least five
       varints of at least a byte each, so what is left in the buffer is the real ceiling and
       no honest code can be over it. */
    if (!(n >= 0) || n * 5 > body.length) throw new Error("not a design link");
